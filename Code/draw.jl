...
function draw_slices(x, y, nx, ny, Lx, Ly, plot_times, m)
    ...
    #plot(x, (U[:,ny]), xlabel = L"$x$", ylabel = L"$u(x,5,t)$", linecolor = :black, linewidth = 2, aspect_ratio = 20.0, grid = false, margin=3mm, label="theta", xlims=(0,Lx), ylims=(0,1))
    for i in plot_times
