...
function draw_heat(x, y, U, V, ϕ, i, Lx, Ly)
    ...
    p1 = heatmap(x,y,transpose(U), xlabel = L"$x$", ylabel = L"$y$", margin=3mm, aspect_ratio=:equal, xlims=(-Lx,Lx), ylims =(0,Ly), tick_direction=:out, c=:plasma, clims=(0.0, 1.0))
    contour!(p1, x, y, transpose(ϕ), c=:red, linewidth = 2, levels=[0.000001], clim=(0.0, 1.0))
    savefig("u-$i.pdf")
    heatmap(x,y,transpose(V), xlabel = L"$x$", ylabel = L"$y$", margin=3mm, aspect_ratio=:equal, xlims=(-Lx,Lx), ylims =(0,Ly), tick_direction=:out, c=:plasma)
    savefig("V-$i.pdf")
    p2 = heatmap(x,y,transpose(ϕ), xlabel = L"$x$", ylabel = L"$y$", margin=3mm, aspect_ratio=:equal, xlims=(-Lx,Lx), ylims =(0,Ly), tick_direction=:out, c=:plasma)
...
function draw_slices(x, y, nx, ny, Lx, Ly, plot_times, m)
    ...
    Φ = readdlm("U-0.csv")
    U = Φ.^(1/(1+m))
    plot(x, U[:,ny], xlabel = L"$x$", ylabel = L"$u(x,5,t)$", linecolor = :black, linewidth = 2, aspect_ratio = 20.0, grid = false, margin=3mm, legend = false, xlims=(-Lx,Lx), ylims=(0,1))
    for i in plot_times
...
function draw_growth(t, Amp, t_min::Float64)
    ...
    poly = fit(t[ind:end-1], log.(Amp)[ind:end-1], 1) # Fit straight line to data
    plot(t, log.(Amp), xlabel = L"$t$", ylabel = L"$\log(A)$", label = "Numerical Data", linewidth = 2, margin = 5mm) # Plot data
    scatter!([t[ind], t[end-2]], [log(Amp[ind]), log(Amp[end-2])], markersize = 5, markershape = :xcross, markercolor = :red, label = false) # Scatter plot of t_min
...
function plot_interface(x, y, dx, Lx, Ly, Nx, Ny, plot_times)
    ...
    for k in pt
        ...
        # Plot interface
        scatter!([xi],[yi], xlabel = L"$x$", ylabel = L"$y$", margin=3mm, xlims=(-Lx,Lx), ylims=(0,Ly), color="green", legend = false, aspect_ratio=:equal, markersize=2)
    end
    savefig("interface.pdf")
end
